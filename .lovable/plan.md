...
- Canonical self-referencing `https://guru-spark-vision.lovable.app/insights/{slug}`
...
- `head({ params, loaderData })` receives the resolved post. Absolute canonical + og:url built from `https://guru-spark-vision.lovable.app/insights/${params.slug}`. `og:image` omitted per rules unless we generate a real per-post asset (we won't this pass — placeholder previews score worse than none).
